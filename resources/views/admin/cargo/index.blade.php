...
@section('ContenidoPagina')
    ...
    <div class="clearfix">
        <div class="pull-left">
            <span class="hint--top  hint--success" aria-label="Registrar nuevo Cargo"><a  href="{{ route('cargo.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Nuevo Cargo</a></span>
        </div>
    ...
    <table id="dynamic-table" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if($item->car_estado)
                        <span class="hint--top  hint--warning" aria-label="Cargo Habilitado"><button class="btn btn-warning btn-xs">Activo</button></span>
                    @else
                        <span class="hint--top  hint--error" aria-label="Cargo Bloqueado"><button class="btn btn-danger btn-xs">Bloqueado</button></span>
                    @endif
                </td>
                <td>{{ $item->created_at }}</td>
                <td>{{ $item->updated_at }}</td>
                <td>
                    <span class="hint--top  hint--info" aria-label="Editar"><a href="{{ route('cargo.edit', $item->id) }}" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a></span>
                </td>
            </tr>
            @endforeach
        </tbody>
